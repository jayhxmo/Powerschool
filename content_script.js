...
function updateClassInfoNames(){
	for(var j = 0; j < classInfo.length; j++){
		classInfo[j] = classInfo[j].substring(0, String(classInfo[j]).indexOf('Details') - 1);
	}

	for(var j = 0; j < classInfoTilda.length; j++){
		classInfoTilda[j] = classInfoTilda[j].substr(1, String(classInfoTilda[j]).indexOf('Details') - 1);
	}
}

getData();
updateClassInfoNames();

//classInfoTilda and classInfo contains the names of all the classes
